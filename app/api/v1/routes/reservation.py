...
@router.patch("/{id}")
async def update_reservation_status(db: DBDep, user: UserDep, update: ReservationUpdate, id: int) -> ReservationRead:
  async with db.begin():
...
async def cancel_reservation(db: DBDep, user: UserDep, id: int):
  async with db.begin():
